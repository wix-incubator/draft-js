...
class DraftEditor extends React.Component<DraftEditorProps, State> {
  ...
  registerFiberNode: (fiberNode: any) => void = (fiberNode: any): void => {
    this._blocksFiberNodes = this._blocksFiberNodes.set(
      fiberNode._reactInternalFiber.key,
      fiberNode,
    );
  };

  restoreBlockDOM: (blockKey: string) => void = (blockKey: string): void => {
    const blockRef = this._blocksFiberNodes.get(blockKey);
    const draftContents = window.draftContentsFiber.child;
    let firstChild = draftContents.child;
    let blockBefore = firstChild;
    while (blockBefore.sibling && blockBefore.sibling.key !== blockKey) {
      blockBefore = blockBefore.sibling;
    }

    if (blockBefore.sibling && blockBefore.sibling.sibling) {
      console.log('restoreBlockDom - replacing sibling', blockBefore);
      blockBefore.sibling = blockBefore.sibling.sibling;
    } else {
      console.log('restoreBlockDom - deleting sibling', blockBefore.sibling);
      blockBefore.sibling = null;
      console.log('draftContentsFiber 1', window.draftContentsFiber.child.child);
    }
